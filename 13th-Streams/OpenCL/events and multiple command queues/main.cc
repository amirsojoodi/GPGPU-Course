...
For example:\n ./multiCq 0 10000 512 4 )"
  ...
  // copy(A.begin(), A.end(), ostream_iterator<double>(cout, " "));  // print
  // all elements
  ...
  auto platform = platforms[platform_number]; // here you can select between
                                              // Intel, AMD or Nvidia
  std::vector<cl::Device> devices;
  platform.getDevices(CL_DEVICE_TYPE_ALL, &devices);
  auto device =
      devices.front(); // here you can select between different Accelerators
  ...
  generate(queues.begin(), queues.end(),
           [&]() { return cl::CommandQueue(context, device); }); // cpp = 2xGOD
  ...
  for (int i = 0; i < cq_count; ++i) { // Perform async device calls
    ...
    // 3rd(offset) param is on device, 4th(amount) and 5th(start void ptr) are
    // on host
    queues[i].enqueueWriteBuffer(buf_src, CL_FALSE, offset_with_size,
                                 cq_elem_size, h_data.data() + offset, nullptr,
                                 ndrange_deps.data());
    queues[i].enqueueNDRangeKernel(kernel, offset_ndrange, global, local,
                                   &ndrange_deps, read_deps.data());
    queues[i].enqueueReadBuffer(buf_target, CL_FALSE, offset_with_size,
                                cq_elem_size, d_output.data() + offset,
                                &read_deps, nullptr);
  }
  ...
  double serial_time =
             chrono::duration<double, milli>(tf_serial - ts_serial).count(),
  ...
  if (location.first == h_output.end()) {
    ...
    cout << "[INFO] Test PASS! No mismatch found!\n"
         << "[INFO] Achived speedup of " << serial_time / ocl_time << "x"
         << endl;
  } else
