...
void third_parallel_histogram(int *data, int *parallel_hist, pthread_t *threads,
                              argument *arguments, int data_size, int hist_size,
                              int num_thread) {
  ...
  for (int i = 0; i < num_thread; i++) {
    ...
    pthread_create(&threads[i], 0, parallel_histogram_third,
                   (void *)&arguments[i]);
  }
  ...
  for (int i = 0; i < num_thread; ++i) {
    ...
    // Problem:
    // Memory allocations/deallocations shouldn't be included in measurements!
